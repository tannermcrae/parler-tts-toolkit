...
def log_pred(
    ...
    """Helper function to log target/predicted transcriptions."""
    # Remove wandb logging - just print to console if needed
    if accelerator.is_local_main_process:
        cur_step_pretty = f"{int(step // 1000)}k" if step > 1000 else step
        prefix_pretty = prefix.replace("/", "-")
        
        logger.info(f"Step {cur_step_pretty} {prefix_pretty} predictions:")
        for i in range(min(3, len(pred_descriptions))):  # Log first 3 examples
            logger.info(f"Description: {pred_descriptions[i]}")
            logger.info(f"Prompt: {pred_prompts[i]}")
            logger.info(f"Transcription: {transcriptions[i]}")
            if si_sdr_measures:
                logger.info(f"Noise estimation: {si_sdr_measures[i]}")
            logger.info("---")
